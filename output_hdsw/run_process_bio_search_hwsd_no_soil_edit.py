...
PROCESS_BIO = Path(r"F:\APSIM710-r4221\process_bio\output_hdsw\output")
INDEX_PATH = PROCESS_BIO / "iteration_index.csv"
BEST_DIR = PROCESS_BIO / "best"
WORK_DIR = PROCESS_BIO / "work"
WHEAT_LATE_STAGE_CN_DEFAULT = "蜡熟期"
DEFAULT_VALIDATION_CSV = Path(r"F:\APSIM710-r4221\process_bio\independent_validation_observations_p02_maize_p01_wheat.csv")
DEFAULT_TRUTH_TEMPLATE = Path(r"F:\APSIM710-r4221\process_bio\output_hdsw\modified_from_truth_hwsd.apsim")
EPS = 1e-9
...
def extract_soil_nodes(tree):
    """
    Extract all Soil-related XML nodes from an APSIM .apsim file.
    This function detects nodes whose tag or Name/name attribute contains:
    Soil, Soils, SoilWater, Water, SoilNitrogen, Analysis, InitialWater, InitialNitrogen.
    """
    ...
    for elem in root.iter():
        tag = _tag_no_ns(elem.tag).lower()
        name_attr = str(elem.get("Name") or elem.get("name") or "").lower()
        if any(k in tag or k in name_attr for k in SOIL_KEYWORDS):
            out.append(ET.tostring(elem, encoding="unicode"))
    return out


def _parse_xml_text(xml_text: str):
    return ET.ElementTree(ET.fromstring(xml_text))


def soil_snapshot(xml_text: str) -> str:
    tree = _parse_xml_text(xml_text)
    parts = extract_soil_nodes(tree)
    return "\n---SOIL_NODE_SPLIT---\n".join(parts)


def assert_soil_unchanged(before_text: str, after_text: str):
    if soil_snapshot(before_text) != soil_snapshot(after_text):
        raise RuntimeError("ERROR: Soil module was modified, this iteration is invalid.")


def is_soil_related_action(action: str) -> bool:
    txt = str(action or "").lower()
    return "soil" in txt or "water" in txt or "nitrogen" in txt
...
def pick_action(k: int, baseline_eval: dict, mode: str):
    ...
    if mode == "conservative":
        cycle = ["wheat_density_only", "wheat_cultivar", "wheat_density_only", "wheat_cultivar"]
        return cycle[k % len(cycle)]
    ...
    if w_tb is not None and m_tb is not None and w_tb > m_tb + 0.12:
        cycle = ["wheat_density_only", "wheat_cultivar", "wheat_density_only", "maize_cultivar", "fertilizer_only"]
    elif w_tb is not None and m_tb is not None and w_tb > m_tb + 0.08:
        cycle = ["wheat_density_only", "wheat_cultivar", "fertilizer_only", "maize_cultivar"]
    else:
        cycle = ["wheat_density_only", "wheat_cultivar", "maize_cultivar", "fertilizer_only"]
    return cycle[k % len(cycle)]
...
def parse_args():
    p = argparse.ArgumentParser(description="Bio-first search (HWSD no-soil-edit mode) with outputs under process_bio/output_hdsw/output")
    ...
    p.add_argument(
        ...
        help="mixed: joint search; wheat_focus: freeze maize and only adjust wheat variables; conservative: wheat_focus with non-soil actions only",
    )
    ...
    p.add_argument(
        ...
        help="ignored in this script; APSIM input is fixed to output_hdsw/modified_from_truth_hwsd.apsim",
    )
    return p.parse_args()
...
def main():
    ...
    fixed_apsim_input = DEFAULT_TRUTH_TEMPLATE
    if not fixed_apsim_input.exists():
        raise FileNotFoundError(f"Fixed APSIM input not found: {fixed_apsim_input}")

    # Force workspace truth path into output_hdsw/output/work, never overwrite fixed APSIM input.
    base.TRUTH_PATH = WORK_DIR / "truth.apsim"
    shutil.copy2(fixed_apsim_input, base.TRUTH_PATH)
    ...
    # In no-soil-edit HWSD mode, always start from fixed APSIM input.
    if args.truth_template and Path(args.truth_template) != fixed_apsim_input:
        print(f"[INFO] --truth_template is ignored in this script. Using fixed APSIM input: {fixed_apsim_input}")
    ...
    for k in range(max(1, int(args.rounds))):
        ...
        skipped_soil_action = False

        if is_soil_related_action(action):
            skipped_soil_action = True
            skip_msg = f"iteration {iter_no}: action={action} skipped because it belongs to Soil module"
            print(skip_msg)
            base.write_text(logs_dir / "soil_skip.log", skip_msg + "\n")
            # Fall back to a non-soil action so iteration can proceed.
            action = "wheat_density_only"
        ...
        if action == "wheat_cultivar":
            ...
            scope = "skipped_soil"
            skipped_soil_action = True
            msg = "skipped because it belongs to Soil module"
            non_cultivar_changes.append({"factor": "soil_only", "status": msg})
            base.write_text(logs_dir / "soil_skip.log", f"iteration {iter_no}: {msg}\n")
        ...
        try:
            assert_soil_unchanged(truth_before, truth_after)
        except RuntimeError as e:
            err = str(e)
            print(err)
            base.write_text(logs_dir / "soil_guard_error.log", err + "\n")
            cand_truth_path = out_cand / "truth.apsim"
            if cand_truth_path.exists():
                cand_truth_path.unlink()
            break
        ...
        manifest = {
            "iteration": iter_no,
            "change_scope": scope,
            "soil_action_skipped": skipped_soil_action,
        ...
        summary = (
            ...
            f"五、结果目录\n"
            f"- process_bio/output_hdsw/output/iter_{iter_no:03d}/\n"
        )
